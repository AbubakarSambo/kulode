...
export function Header({ title, description, action, icon: Icon, badgeText, category }: HeaderProps) {
  return (
    <>
      ...
      {/* The header above is desktop-only (hidden below `sm`) — without this, any "Add"/"Create"
          action passed in only via the `action` prop would be completely unreachable on mobile. */}
      {action && <div className="border-b border-border p-4 sm:hidden print:hidden">{action}</div>}
    </>
  )
}
