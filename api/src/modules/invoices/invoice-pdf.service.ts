  ...
  async generatePdf(invoice: InvoiceData): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      if (invoice.organization.email) {
        doc.text(invoice.organization.email, 50, yPos);
        yPos += 14;
      }
      ...
      if (invoice.organization.address) {
        doc.text(invoice.organization.address, 50, yPos, { width: 200 });
      }
      ...
      doc
        ...
        .fontSize(12)
        .font('Helvetica-Bold')
        .text(invoice.client.name, 50, 188);
      ...
      if (invoice.client.email) {
        doc.text(invoice.client.email, 50, yPos);
        yPos += 14;
      }
      ...
      if (invoice.client.address) {
        doc.text(invoice.client.address, 50, yPos, { width: 200 });
      }
      ...
      doc
        ...
        .fontSize(10)
        .font('Helvetica-Bold')
        .text('ISSUE DATE', 350, 170)
        .text('DUE DATE', 450, 170);

      doc
        .fillColor(textColor)
        .fontSize(10)
        .font('Helvetica')
        .text(this.formatDate(invoice.issueDate), 350, 188)
        .text(this.formatDate(invoice.dueDate), 450, 188);
